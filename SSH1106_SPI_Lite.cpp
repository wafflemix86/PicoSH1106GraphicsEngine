...
static uint8_t _blankframe[SSH1106_LCDHEIGHT*SSH1106_LCDWIDTH/8] = {0};
...
void SSH1106::drawPixel(uint8_t x, uint8_t y, uint8_t color)
{
    switch (color) 
    {
      case 0: buffer[x+ (y >> 3)*SSH1106_LCDWIDTH] &= ~(1 << (y&7)); break; //black
      case 1: buffer[x+ (y >> 3)*SSH1106_LCDWIDTH] |=  (1 << (y&7)); break; //white
      case 2: buffer[x+ (y >> 3)*SSH1106_LCDWIDTH] ^=  (1 << (y&7)); break; //invert
    }
}
...
void SSH1106::setBuffer(uint8_t* buff)
{
    // memcpy(buffer, buff, SSH1106_LCDHEIGHT * SSH1106_LCDWIDTH / 8);
    buffer = buff;  // woop woop dangerous pointer assignment.
}
